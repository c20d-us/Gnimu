...
- [x] **Migrated to the generalized axis remap (2026-08-14).** `IMU_SWAP_XY` / `IMU_SIGN_X/Y/Z` are retired in favour of `IMU_AXIS_{X,Y,Z}_SRC` / `_SIGN`, the scheme developed in the [nRF52840-OLED tree](../Gnimu-nRF52840-OLED/DESIGN.md) §6. Each vehicle axis now names which sensor axis feeds it plus a sign, covering all **24** physically-realizable orientations instead of the old model's 8 flat ones, and `remapAxes()` in `g_imu.cpp` became a copy-then-permute (a general permutation cannot be done in place).

  **This was a behavioral no-op.** The as-built map above — `SWAP_XY false` with `−1, −1, +1` — converts mechanically to order `XYZ` with the same signs: even permutation, two sign flips, determinant +1. Nothing about the shipped orientation changed; only how it is spelled.

  **The reason to do it anyway is the guard that came with it.** The old four-macro model had no way to express the mirror check, and this config is precisely where that cost was paid: the determinant −1 bug described above survived a drive test and an app-side investigation before arithmetic caught it. The new form carries a compile-time `IMU_AXIS_PARITY` determinant `static_assert`, so that entire class of error is now impossible to ship — an even permutation must have an even number of sign flips, an odd one an odd number, and anything else fails the build with a message naming the problem.

  Two side effects worth recording. `g_imu.cpp` is now **byte-identical to the OLED tree's**, the axis scheme having been its last difference, so it is enforced by `check_common.sh`'s nRF group rather than merely happening to match. And with the ESP32 tree migrated in the same pass, `IMU_SWAP_XY` no longer exists anywhere in the codebase — including the diagnostic sketches, which now report in the current form.
