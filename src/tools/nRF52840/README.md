...
| [`imu_calibration/`](imu_calibration/imu_calibration.ino) | Per-axis IMU zero-point offsets in the raw sensor frame, independent of `IMU_SWAP_XY`/`IMU_SIGN_*` (feeds `config.h`'s `IMU_ACCEL_OFFSET_*`/`IMU_GYRO_OFFSET_*`) | level bench surface | Hands-off: 5-minute warmup, then repeating 10000-sample sessions (1 min apart) each printing six paste-ready `IMU_*_OFFSET_*` `#define` lines plus deltas vs the previous session; use the values once consecutive sessions agree. |
...
| [`gnss_en/`](gnss_en/gnss_en.ino) | GNSS rail EN-pad cutoff: does D8 LOW actually disconnect the TPS63020 output? (`g_battery.cpp` low-voltage cutoff + planned GNSS idle-cutoff) | TPS63020 + GNSS wired, multimeter | In the LOW state the TPS63020 output drops to ~0 V **and** GNSS UART goes silent (0 bytes). *(Confirmed — with a caveat, see below.)* |
