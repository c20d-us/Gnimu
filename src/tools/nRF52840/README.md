...
|---|---|---|---|
| [`imu_probe/`](imu_probe/imu_probe.ino) | Onboard LSM6DS3TR-C power pin, library bring-up, units (`g_imu_lsm6ds3.cpp`) | none | `begin() OK`; resting board reads ~+1 g on one accel axis (total ~1 g, not ~9.8) and ~0 dps gyro. *(Confirmed.)* |
...
`GNSS_BAUD` / `BLE_TX_POWER_ADV_DBM` for its thermal load. Note `imu_calibration`
still sets the Seeed library's `accelBandWidth`, which the firmware stopped
