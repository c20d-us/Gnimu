...
// connection, and no sky view is required. On the nRF52840-OLED build set
// CAL_DISPLAY_ENABLED to 1 to add the panel, which is continuously lit in
// that variant's production firmware and is therefore part of the load the
// die settles against; it doubles as a USB-free readout of phase,
// temperature and the latest offsets.
//
...
// only needed to retrieve results (or not even that, on the OLED build).
// ============================================================================
...
// ----------------------------------------------------------------------------
// OLED panel - SET THIS TO 1 FOR THE nRF52840-OLED BUILD, 0 for the plain one.
//
// On that variant the panel is lit continuously in production, so it is part
// of the load the die settles against; leaving it dark here would calibrate a
// cooler board than the firmware ever runs on. It also makes this tool
// readable with NO USB ATTACHED - phase, die temperature and the latest
// offsets all appear on screen, which is the whole point of an unattended
// run.
//
// Deliberately a flag rather than an I2C probe, because the two boards
// disagree about what A4 IS: SDA on the OLED build, POWER_SWITCH_SENSE_PIN
// (an analog tap on a resistor divider) on the plain one. Auto-probing would
// mean driving that divider as an I2C output on the wrong board - harmless at
// these impedances, but not something to do behind your back. Same footgun
// config.h warns about for the main firmware; here the flag makes it a
// choice. If the flag is set but no panel answers, the probe below disables
// the display rather than hanging.
// ----------------------------------------------------------------------------
#define CAL_DISPLAY_ENABLED 1
#define DISPLAY_I2C_ADDRESS 0x3C
#define DISPLAY_CONTRAST 255
#define DISPLAY_REFRESH_MS 1000

// Included here rather than at the top because the guard above has to be
// defined first. The IMU library drives Wire1 internally, so the panel's Wire
// is a separate bus and the two never contend.
#if CAL_DISPLAY_ENABLED
#include <U8g2lib.h>
#include <Wire.h>
...
// ----------------------------------------------------------------------------
// OLED status panel (nRF52840-OLED build only). Two jobs: reproduce the
// panel's share of production's thermal load, and make an unattended run
// legible without USB.
//
...
static void displayBringUp() {
  ...
  // Probe before committing, so a wrong CAL_DISPLAY_ENABLED or an unplugged
  // panel costs a warning rather than a hang.
