...
### IMU read path: bus speed and burst reads

`imuPoll()` samples all six axes every `IMU_SAMPLE_INTERVAL_MS` (10 ms, 100 Hz), which made it the largest recurring blocking cost in `loop()` — larger per occurrence than the display's metered I2C slices, and running three times as often. Two problems, both found 2026-08-06 while auditing loop latency:

**The IMU bus was running at 100 kHz.** The core's `TwoWire::begin()` hardcodes `FREQUENCY` to `K100`, and the Seeed LSM6DS3 library calls `begin()` but never `setClock()` — so the IMU's bus (`Wire1`/TWIM1) sat at a quarter the speed of the display's (`Wire`/TWIM0), which had been explicitly configured to 400 kHz. Nothing surfaced this; it is simply the default nobody had reason to look at. Fixed with `IMU_I2C_CLOCK_HZ` (400 kHz), applied in `configureNormalMode()` **after** `myIMU.begin()` — the ordering is required, since `begin()` resets the frequency register, and it must re-apply on every call because `imuDisarmWake()` runs `begin()` again on each LIGHT_SLEEP exit.

**Each axis was two separate I2C transactions.** `readFloatAccelX()` and friends each issue a register-address write followed by a 2-byte read, so a six-axis sample cost **twelve transactions** — despite the six output registers being contiguous and the library exposing a `readRegisterRegion()` capable of fetching them in one go. `readImuRaw()` now does a single 12-byte burst from `OUTX_L_G` (0x22) through `OUTZ_H_XL` (0x2D): **two transactions instead of twelve**.

Together these took the sample tick from roughly 3 ms to about 0.3 ms — calculated from bus arithmetic, not measured, but an order of magnitude either way.

**The burst read is a correctness fix, not only a speed one — and this is the reason to keep it even if latency stopped mattering.** BDU (enabled in `configureNormalMode()`) freezes each register *pair* so a 16-bit read cannot straddle a sample. It does nothing about the gaps *between* six separate transactions, where a new sample can land — so X could come from one sample and Z from the next. A single burst returns one coherent six-axis set. That matters here specifically because those values feed per-chip offset correction, an axis remap, and a transient-peak detector whose whole job is spotting short events: a torn sample is exactly the kind of artefact it would latch onto and report as real.

The read now also **reuses the previous sample if the transfer fails**, rather than feeding a failed read into the filters as the per-axis path silently did. Deliberately silent — this runs at 100 Hz, and logging per failure would be its own latency problem on a flaky bus. The tradeoff is that a failing bus becomes invisible rather than merely harmless; a counter surfaced on the display would be the way to regain visibility if that ever matters.

Applies to both nRF52 variants. **Not** the ESP32 build — different sensor (MPU-6050), different driver, and the library's `Wire`→`Wire1` remap is guarded to the XIAO targets. Whether that build has an equivalent 100 kHz default is untested; see Open items.
...
**⚠️ Superseded 2026-08-14 (as-built):** the end-wall mount is *not* what shipped. The XIAO ended up mounted **flat, USB-C rearward, SoC face up** — the same flat class as the prior builds, and an identity map (order `XYZ`, no sign flips). So the motivating case above never materialized, and nothing in this build currently *requires* the generalized scheme.

**The generalized scheme was kept anyway, and this section is retained as the rationale for it.** It is implemented, bench-correct, and strictly subsumes the old four-macro model — the migration table below maps any old config onto it mechanically. Retiring it to go back to `IMU_SWAP_XY` would be churn that buys nothing and would re-introduce the mirror-map class of bug that the determinant `static_assert` now catches at compile time. It also leaves the edge-mount option open if the mechanical layout moves again, which it has once already.
...
- [ ] **Retest multi-constellation (Galileo) now that the IMU bus runs at 400kHz.** All three variants have historically failed to hold 25Hz with anything beyond GPS enabled — the base tree's README documents the M100-5883 sagging to ~24Hz above ~12 SVs with GPS+Galileo, and `GNSS_CONSTELLATIONS` has shipped GPS-only ever since. That conclusion was reached with the IMU I2C bus at the core's 100kHz default, i.e. with `imuPoll()` spending roughly ten times longer per sample tick than it needs to (twelve transactions every `IMU_SAMPLE_INTERVAL_MS`, ~30% loop duty by calculation). **The multi-constellation ceiling may have been a firmware latency budget problem rather than a receiver capability limit** — or at least partly so. Worth re-running with Galileo enabled: more SVs would materially improve pDOP and hAcc, which matters for this device's actual use. Note this cuts against the M100 Mini's smaller antenna, so a negative result is not conclusive for the -5883.
- [x] **ESP32 variant investigated and updated (2026-08-06).** Only half the nRF52 fix applied. **Burst read: already correct** — `Adafruit_MPU6050::_read()` fetches all six axes plus temperature as a single 14-byte burst from `MPU6050_ACCEL_OUT`, so the twelve-transactions-per-tick problem never existed there. **Bus clock: same latent issue** — `Wire.begin()` leaves the ESP32 bus at 100kHz, and while `Adafruit_BusIO` exposes `setSpeed()`, the MPU-6050 library never calls it. Fixed with `IMU_I2C_CLOCK_HZ` applied after `myIMU.begin()`. Motivation is the same as on the nRF52 boards: Galileo had been dropped from that variant too because it could not hold 25Hz with GPS+Galileo, and the freed loop headroom may be enough to re-enable it.
- [ ] **Retest GPS+Galileo on the ESP32 variant** now that its IMU bus runs at 400kHz — same hypothesis as the nRF52 trees, and the reason the clock change was made there at all.
- [x] **ESP32 GNSS baud deliberately left at 115200 (2026-08-06).** The nRF52 trees dropped to 57600 to widen the RX-buffer fill window, but that constraint does not exist here: ESP32's `HardwareSerial` defaults to a **256-byte** RX buffer against the nRF52's 64, so its window at 115200 (~22ms) is already wider than 57600 buys on the nRF52 (~11ms). Lowering it would spend link headroom to solve a problem this variant does not have.
...
- [x] **Generalized axis remap implemented (2026-08-11).** `IMU_AXIS_{X,Y,Z}_SRC`/`_SIGN` in `config.h` with the order-name table and the static-pose derivation procedure, the `IMU_AXIS_PARITY` determinant `static_assert` plus per-macro range/sign asserts, `IMU_SWAP_XY`/`IMU_SIGN_*` retired from this tree, and `remapAxes()` in `g_imu.cpp` rewritten as a copy-then-permute (a general permutation can't be done in place). This tree only — see the base-tree migration item below.
- [x] **This build's axis map derived (2026-08-14): order `XYZ`, no sign flips** — the identity map. The end-wall mount was abandoned and the XIAO sits **flat, USB-C rearward, SoC face up**; see the superseding note in [§6](#why-the-existing-model-runs-out). Even permutation with zero sign flips, determinant +1, so the handedness assert passes. Note this legitimately differs from the base tree's `−1, −1, +1`, which reflects its own USB-C-**forward** as-built orientation — there is no shared-value expectation between trees.
- [x] **`imu_calibration` re-run and offsets landed (2026-08-14).** Six per-chip offsets pasted into `config.h` from the aggregated multi-session run. **The in-case trims were not carried over**: the base tree layers a further +0.0075g on Z accel and +0.045dps on Z gyro derived from full-firmware in-case behavior the bench sketch can't observe, and whether this board needs the equivalent is untested. The mount ended up flat like the base tree's, so the orientation-dependence that motivated re-deriving the Z trim did not materialize — but the load-dependence still applies. Flagged inline in `config.h`; verify resting Z accel and yaw rate in-case before treating these as final.
- [x] **`imu_calibration` split into a per-tree copy (2026-08-14).** This tree now carries [its own](../tools/nRF52840-OLED/imu_calibration/imu_calibration.ino) rather than the base tree's copy taking a `CAL_DISPLAY_ENABLED` build flag. The flag was the wrong shape: it made a sketch that had to be edited before use on either board, and the two boards disagree about what A4 *is* (SDA here, the switch-sense divider tap there), so the wrong setting was a real footgun rather than a cosmetic one. Splitting lets each copy hardcode its own variant — the panel is unconditional here (thermal load **and** a USB-free readout, which is what makes an unattended run retrievable without plugging in), and BLE advertises at this tree's `-20` dBm rather than the base's `-16`. **The measurement/storage/aggregation core is byte-identical between the two copies and must stay that way** — that is what keeps results comparable across trees; a change to the sampling, gate or aggregation logic has to be applied to both. Note this is now a *three*-way duplication: the ESP32 tree has [a copy](../tools/ESP32/imu_calibration/imu_calibration.ino) too, same approach in that sensor's native m/s² and rad/s.
- [x] **`imu_tiltmap` stays single-sourced in the base tree (decided 2026-08-14).** Not copied here, and not because the duplication would be costly — it has no display code and no per-variant settings, so a copy would have been nearly free. It is that **the sketch is largely redundant**: the production firmware already prints the 1 Hz serial `milliG` line, and the three static poses in `config.h`'s derivation procedure fully determine the map from it. That is not theoretical — it is exactly how the base tree's as-built map was settled on 2026-08-04, in preference to a drive test. Running the real firmware also has the advantage of exercising the map you are actually shipping, offsets and all, rather than a bench sketch's parallel copy of the settings. **Caveat, and it is the whole reason this is worth writing down: derive against the raw serial `milliG` numbers, NOT the Gnimu Monitor readout.** Monitor is a display layer that has been wrong about exactly this before — a label-binding bug that masked a mirrored (determinant −1) map for some time; see the base tree's [DESIGN.md](../Gnimu-nRF52840/DESIGN.md) §4 and its open items. It is fine for confirming the device is alive and streaming; it cannot validate firmware signs. `imu_tiltmap` remains in the base tree's `tools/` as the from-scratch path for a board whose sensor orientation is unknown.
- [x] **All three trees migrated to the generalized remap (2026-08-14).** The question is settled by convergence rather than by accepting divergence: `IMU_SWAP_XY`/`IMU_SIGN_*` are now **retired codebase-wide**, and the base nRF52840 and ESP32 trees both carry `IMU_AXIS_*_SRC`/`_SIGN`, the full assert set (per-axis range, per-axis sign, permutation-collision, and the `IMU_AXIS_PARITY` determinant check) and the copy-then-permute `remapAxes()`. Both conversions were mechanical and **behaviorally identity**: base `SWAP_XY false`, `−1, −1, +1` → order `XYZ` with the same signs; ESP32 identity → order `XYZ`, `+1, +1, +1`. Each is an even permutation with an even number of sign flips, determinant +1, so both pass the new mirror check that the old model could not express. **Dividend: `g_imu.cpp` is now byte-identical across both nRF trees** — the axis scheme was its last divergence — so it has joined the enforced nRF group in `check_common.sh`, taking that group from 9 files to 11. The remaining deliberate nRF divergences are down to three: `g_led.cpp`, `g_state.cpp`, `g_power.h`.
...
- [x] **ESP32 axis signs confirmed correct (2026-08-06).** Carries the identity map, which is a valid proper rotation (even permutation, zero sign flips, determinant +1). Different enclosure and a different sensor orientation from the nRF52 builds, so the values legitimately differ from their `−1, −1, +1`; there is no shared-value expectation between trees here. **Updated 2026-08-14:** this tree was migrated to `IMU_AXIS_*_SRC`/`_SIGN` along with the base tree, so the identity map is now spelled order `XYZ` with `+1, +1, +1`. Worth noting this variant is the one that benefits most from the determinant assert: its MPU-6050 is a separate module oriented by hand, not a sensor fixed to a known board.
