...
- [ ] Decide whether to migrate the **base nRF52840 tree** to the same remap scheme, or accept the divergence (§6, *Where this should live*). **Status as of 2026-08-14: the generalized scheme is in this tree ONLY — one of three.** Both the base nRF52840 and the ESP32 still carry `IMU_SWAP_XY`/`IMU_SIGN_*`, so `g_imu.cpp`'s `remapAxes()` and the axis block of `config.h` are now the largest deliberate divergence in the codebase. Neither lagging tree is broken — both mount flat, which the old model covers — and both convert mechanically via the migration table in §6 (base: `SWAP_XY false`, `−1, −1, +1` → order `XYZ`, same signs; ESP32: identity → order `XYZ`, `+1, +1, +1`). Decide deliberately rather than letting it drift by default.
...
- [x] **ESP32 axis signs confirmed correct (2026-08-06).** Carries `IMU_SWAP_XY false` with `+1/+1/+1` — the identity map, which is a valid proper rotation (even permutation, zero sign flips, determinant +1). Different enclosure and a different sensor orientation from the nRF52 builds, so the values legitimately differ from their `-1/-1/+1`; there is no shared-value expectation between trees here. Note this variant still uses the older `IMU_SWAP_XY` model rather than the generalized `IMU_AXIS_*_SRC`/`_SIGN` scheme — fine while its mount stays flat, but it would need migrating before any edge mount.
