...
| [`Gnimu-nRF52840/`](Gnimu-nRF52840/) | The battery-powered evolution. Seeed XIAO nRF52840 Sense with its onboard LSM6DS3TR-C IMU, LiPo charge/state-of-charge subsystem, state machine, RGB status LED. | [README](Gnimu-nRF52840/README.md) |
...
g_gnss_port.h           The seam between th GNSS driver and the MCU's UART
...
path, so a library cannot see `config.h`. The library approach was tried (`GnimuCore`) and
...
than the duplication it replaced. Distribution is a secondary cost: a library
has to be installed into the sketchbook before any variant will build, where
a sketch folder just opens and builds.
...
`check_common.sh` would still be needed for the rest.
...
here is compiled into the firmware; these exist to validate assumptions, perform one-off measurements or actions, or
otherwise exercise code functions prior to use.
