...
Design reference for **Gnimu Pro**, an evolution of the ESP32-based Gnimu RaceBox Mini emulator, re-targeted to a **battery-powered Seeed Studio XIAO nRF52840 Sense**. The name signals "evolution with more features"; it is cosmetic only (see [Protocol identity](#protocol-identity--compatibility)).

> Status: new hardware on the breadboard (XIAO + M100 + TPS63020 + 900 mAh LiPo + slide-switch + switch-sense divider on A4), all subsystems bench-validated. Firmware port re-done from scratch onto the optimized Gnimu Classic codebase (prior port archived under `src/archive/Gnimu-Pro-v1/`). Streaming pipeline confirmed rock-solid at 25 Hz end-to-end (RaceBox-app compatible, real fix, IMU streaming). **State machine complete and in code, Phase 1 + Phase 2 both bench-validated**: RUNNING / CHARGE_ONLY / BATTERY_WAIT / LIGHT_SLEEP / DEEP_SLEEP orchestrated by `gp_state`, mechanisms in `gp_power` (rail EN, switch-sense, USB detect, System OFF) and `gp_gnss`/`gp_imu` (per-subsystem low-power entry points), sensing in `gp_battery`, LED in `gp_led`. Switch-sense on A4 (bench-confirmed 0 mV ON / ~2085 mV OFF), boot classifier + all runtime transitions verified, including the full three-tier LIGHT_SLEEP → DEEP_SLEEP escalation and both wake triggers (BLE connect, IMU shake). GNSS navigation-database persistence (warm-start) was built, bench-validated, then pruned — see [§4](#gnss-navigation-database-persistence-warm-start--removed-2026-07). Remaining: an in-car drive test to reconfirm `IMU_SWAP_XY` / `IMU_SIGN_*` for the new mount. This document is the canonical design reference — update it as decisions change.
...
| GNSS | **HGLRC M100 (M100-5883, u-blox M10)** | Reused from v1; good real-world performance. Must stay **u-blox/UBX** — the whole `gp_gnss.cpp` (SparkFun u-blox lib, NAV-PVT) only works with u-blox silicon. |
...
- **XIAO MG24 Sense** — cheaper, newer (Silicon Labs EFR32MG24, Matter/Thread, AI accelerator), but a *different BLE ecosystem*. Would force redoing the BLE layer (the core of this project) on a less-mature stack, plus rework of sleep/bootloader/battery-sense. Its extra features are unused here.
...
- **NEO-F10N / DAN-F10N** — dual-band L1/L5, better multipath rejection at tracks, but *verify it sustains 25 Hz* (F10 is asset-tracking oriented).
...
- **Switching noise vs GNSS:** keep the buck-boost + inductor away from the GNSS antenna; add ~100 µF bulk cap at GNSS VCC; add a ferrite/LC on the GNSS supply if fixes degrade. Layout matters more than the PS mode setting.
...
That leaves **X and Y in the horizontal plane = vehicle forward/lateral**. The XIAO Sense's in-plane axis directions/signs are **not officially documented** (confirmed via Seeed wiki + forums), so which of X/Y is forward vs. lateral, and their signs, still need the empirical **1 g / drive test in the installed orientation**. In practice this reduces to a possible **X↔Y swap and/or sign-flip of X and Y** — the full 3-axis permutation the end-wall mount would have required is no longer needed now that Z falls out naturally. **Apply the identical remap to the gyro.** This simple swap/sign-flip is valid only because the board is mounted **square** (orthogonal) and level in a consistent, known orientation — keep it that way.

### GNSS navigation database persistence (warm-start) — REMOVED, 2026-07

> **Status: implemented, bench-validated end-to-end, then pruned from the main tree.** After a day of real-world observation, the added complexity (a new `gp_storage` module, QSPI+LittleFS glue, a hand-wired flash device table, CRC-checked file format, three new force-save call sites in the state machine, a required one-time flash-provisioning step) wasn't earning its keep against the actual value delivered — a warm-start TTFF improvement that only matters once GNSS is actually being power-cycled, which at the time didn't happen yet (this feature's original motivating use case was Phase 2 LIGHT_SLEEP, which hadn't been built). LIGHT_SLEEP has since been built (see [§5](#5-state-machine)) — but it turned out to need no flash-based warm-start at all: GNSS uses RXM-PMREQ **backup mode**, not a full power cycle, and its ephemeris/almanac survive that transition in the receiver's own BBR without any help from us (the receiver's `VAL_LAYER_RAM_BBR`-persisted config settings need the same protection, which `gp_gnss.cpp` handles directly — see the LIGHT_SLEEP write-up below). So the motivating case this feature was built for turned out not to need it after all, and it remains removed. The one-time parallel copy that had been kept for reference (`src/Gnimu-Pro-GNSSdb/`) has since been deleted — it was confirmed byte-for-byte identical to what's already permanently preserved in git history (commit `34b23bd`), so nothing was lost; `git show 34b23bd:src/Gnimu-Pro/gp_storage.cpp` (or checking out that commit) recovers it if ever needed. The write-up below is left as-is as a design record of what was built and why — none of the file paths or function names it mentions exist in the current `src/Gnimu-Pro/` tree anymore.

`gp_gnss` persists the receiver's navigation database (ephemeris + almanac, via UBX-MGA-DBD) to flash and re-injects it on every boot, so a warm-start recovers a fix in seconds instead of paying a full ~30 s cold-start TTFF every time. Landed 2026-07 as the enabling groundwork for Phase 2's LIGHT_SLEEP (which will hard-cut the GNSS rail rather than keep it hot).

**Save triggers** (all go through `gnssSaveDb()`, which self-guards on fix quality first, then an age throttle unless `force=true`):
- Periodic during RUNNING when `!bleIsConnected()` (crash-resilience — a normal, non-force save every `GNSS_DB_SAVE_INTERVAL_MIN`, checked every loop but cheap: the fast path is a single `uint32` compare).
- `force=true` on every runtime transition out of RUNNING (`enterBatteryWait()`, `enterChargeOnly()`, RUNNING→DEEP_SLEEP) — best-effort capture of the freshest possible data before the receiver goes offline for an unknown duration. On a battery-only switch-off the MCU may die before the write completes; LittleFS's journaling means the previous file survives intact if so.
- (Phase 2, not yet built) LIGHT_SLEEP entry, `force=true` — same rationale, now the actual GNSS-rail-cut case this feature exists for.

**Fix-quality gate** (`gnssIsFixQualityGood()`): `validTime && fixType==3 && numSV >= GNSS_DB_MIN_SV && hAcc <= GNSS_DB_MAX_HACC_MM && tAcc <= GNSS_DB_MAX_TACC_NS` (config.h). A partial or inaccurate dump would poison the next boot's warm-start, so this gate applies even to forced saves — `force` only bypasses the age throttle, never the quality check.

**Load** happens unconditionally at the end of `gnssBegin()` (no age check possible at cold boot — no clock until GNSS has a fix). Considered and rejected a delete-on-transition scheme to keep the on-flash copy always-fresh; the receiver already handles stale MGA data gracefully (rejects expired ephemeris after establishing real time, keeps using the still-valid almanac), so a week-old dump costs roughly the same as a fresh cold-start rather than actively confusing the receiver — not worth the added transition-matrix complexity. `pushAssistNowData(true, ...)` (`skipTime=true`) is mandatory — without it, any embedded MGA-INI-TIME message would set the receiver's clock from our stale saved timestamp.

**On-disk format** (`/gnss_db.bin`): 20-byte header (`magic`, `version`, `utcSecs`, `dataLen`, `crc32`) + raw DBD payload. CRC32 (bitwise IEEE 802.3, payload only) catches bit-rot/partial-write corruption that a byte-count check alone can't. `GNSS_DB_MAX_BYTES` = 64 KiB, sized for headroom up to all six M10 constellations enabled (current default GPS+Galileo is ~10-20 KB) rather than just today's config — a saturated dump failing silently in a no-logging production build would be effectively undetectable otherwise.

**AssistNow Autonomous** (`setAopCfg(1)` in `gnssBegin()`) is enabled alongside DBD persistence — an independent, additive mechanism where the receiver extrapolates its own predicted ephemeris from live observations. Free once configured, no runtime cost.

**Storage layer** (`gp_storage`) wraps the XIAO Sense's onboard 2 MiB QSPI flash (Puya P25Q16H) via a hand-wired `lfs_config` bound to `Adafruit_SPIFlash`'s block API — `Adafruit_LittleFS`'s no-arg constructor only targets internal MCU flash, and there's no ready-made "external QSPI" overload. (Internal flash was considered and rejected: only ~28 KiB total, too tight for constellation-expansion headroom.) Adafruit's bundled flash-device database doesn't include Puya either (added upstream after this Arduino core's version was cut), so the P25Q16H device table is supplied explicitly, transcribed from upstream's `flash_devices.h`.

**`gp_storage` never formats flash.** `storageBegin()` only ever attempts a read-only mount; on failure it logs once and leaves storage uninitialized for the rest of the boot, and `storageReadFile`/`storageWriteFile` degrade to safe no-ops (0/false). Firmware runs completely normally without persistent storage — GNSS just cold-starts every boot. Formatting is a deliberate, explicit, offline, one-time step: flash and run [`storage_check`](src/tools/storage_check/storage_check.ino), which reports what's actually on the chip read-only (Phase 1) and only formats after typing `FORMAT` to confirm (Phase 2). **Required before first use on any new board**, and again if the QSPI chip is ever swapped for a different part on a future hardware revision (the device table in both `gp_storage.cpp` and `storage_check.ino` would need updating together — intentionally not shared code, since diagnostics and firmware stay isolated by design).

## 5. State machine

Gnimu Classic ran on always-present USB — "powered ⇒ everything on at full rate" was the only mode, so `setup()`/`loop()` encoded it implicitly. Pro adds a battery, a hardware switch, and a low-power runtime target, which multiplies the meaningful *operating modes* — not features. To make that behavior auditable rather than emergent, Pro replaces the ad-hoc reflexes with an explicit state machine owned by a top-level orchestrator (`gp_state`). Every state change routes through one transition table, and each state's peripheral posture is a single entry action.

### States

| State | GNSS | IMU | BLE | LED | Live? |
|---|---|---|---|---|---|
| **RUNNING** | hot (25 Hz PVT) | on, polling | fast adv + conn | normal priority | yes |
...
2. Bring up `gp_battery` sensor and `gp_power` sense pins. Prime the sampler with one blocking run (~50 ms) to establish an initial VBAT peak; read USB presence and switch position.
3. Classify:
...
**Wake sources:** LIGHT_SLEEP → RUNNING is triggered by BLE connect **or** an IMU wake event (omni-directional shake, not directional tap — see below). Because LIGHT_SLEEP is not System OFF, `loop()` keeps running, so this doesn't need a true hardware interrupt wake — the LSM6DS3TR-C's embedded wake-up detector runs on-chip in low-power accel mode and asserts `IMU_INT1_PIN`, which `gp_state` polls each loop via `imuWakeTriggered()`. The IMU is **not** fully powered down in LIGHT_SLEEP (that would kill wake capability) — `imuArmWake()` drops it to a low-power ODR (`IMU_WAKE_CTRL1_XL`, default 12.5 Hz) with the wake interrupt armed instead.

Three hardware/ordering lessons from bringing this up, all in `gp_imu.cpp`:
...
- **Orientation (revised):** XIAO mounted to the **underside of the lid, flat and right-side-up**, header pins pointing **down** into the case. **USB-C exits the end wall opposite the M100** (GNSS). Two wins over the original end-wall plan: the board sits level, so **Z is the vertical axis and the IMU remap simplifies** (see [IMU axis mapping](#imu-axis-mapping)); and it puts the BLE/USB-antenna end at the far end from the GNSS antenna — good RF separation. Still mount **square** so the remap stays a simple swap/sign-flip.
- **Case cutouts:** USB-C cutout in the **end wall opposite the GNSS** (size for the plug housing, not just the port). With the board right-side-up, the RGB status LED, charge LED, and reset button face the **lid** — put viewing/access holes there, accounting for the shield standoff gap.
...
Several module-level assumptions are checked by the standalone sketches in [`src/tools`](src/tools/) (XIAO + USB, no other parts) — referenced inline below.

- [x] TPS63020 **true disconnect** on EN-low **confirmed** via [`gnss_en`](src/tools/gnss_en/gnss_en.ino): GNSS goes silent and the M100 power LED goes fully dark once the RX back-feed is removed — **no load switch needed**. Caveat found: a bare EN cut phantom-powers the GNSS through RX (idle TX D6 at 3.3 V via the RX ESD diode), so `gnssEnable(false)` must idle TX low too.
...
- [x] IMU **bus / power / address / units confirmed** via [`imu_probe`](src/tools/imu_probe/imu_probe.ino): `PIN_LSM6DS3TR_C_POWER` (pin 15) HIGH enables the IMU; the stock Seeed LSM6DS3 library reaches it out of the box (no manual `Wire`/`Wire1` setup); address `0x6A`; library returns g / deg·s⁻¹ directly. `gp_imu.cpp` updated to match.
- [x] IMU **X/Y forward-vs-lateral assignment + signs confirmed** via a real in-car accelerate/brake/turn test (and the equivalent static bench pose) on both Gnimu Classic and Gnimu Pro: both check out clean with **no firmware changes** — `IMU_SWAP_XY = false`, `IMU_SIGN_X/Y/Z = +1.0f` (the untouched defaults) are correct. Convention: +X = forward/accelerating, +Y = left (ISO 8855-style). The swap symptom that prompted this investigation turned out to be a **Gnimu Monitor app bug** (Lateral/Longitudinal display labels bound to the wrong packet fields), not a firmware issue — now fixed in the app.
- [x] RGB LED **active-LOW polarity and status colors confirmed** via [`led_check`](src/tools/led_check/led_check.ino): OFF goes fully dark and every color matches its label, so `config.h` `LED_ACTIVE_LOW = 1` and the `LED_*_PIN` mapping / `gp_led.cpp` `setLed()` are correct as-is.
- [x] BLE advertising name, TX power, and **MTU ≥ 91** confirmed via [`ble_mtu`](src/tools/ble_mtu/ble_mtu.ino) (MTU 23→247) and the full firmware (streams 25 Hz to a connected client) — `gp_ble.cpp`'s `configPrphBandwidth(BANDWIDTH_MAX)` + central-driven `BLEUart` notify path validated.
